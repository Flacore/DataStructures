...
namespace structures
{
	...
	inline void Treap<K, T>::insert(const K & key, const T & data)
	{
		...
		BSTTreeNode* binaryTreeNode = new BSTTreeNode(treapItem);
		if (tryToInsertNode(binaryTreeNode)) {
			while (!isHeapOK(binaryTreeNode)) {
				if (binaryTreeNode->isLeftSon()) {
					rotateRightOverParent(binaryTreeNode);
				}
				else {
					rotateLeftOverParent(binaryTreeNode);
				}
			}
			size_++;
		}
